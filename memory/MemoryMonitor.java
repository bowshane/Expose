...
/*
 * @(#)MemoryMonitor.java	1.37 20060829
...
public class MemoryMonitor
	extends JPanel
	{
	static JCheckBox cbLogUsage = new JCheckBox("Log Usage");
	public Surface surf;
	JPanel controls;
	boolean doControls;
	JTextField tf;
...
//		setBorder(new TitledBorder(new EtchedBorder(), "Memory Monitor"));
		...
		label.setFont(font);
		label.setForeground(BLACK);
		controls.add(cbLogUsage);
		cbLogUsage.setFont(font);
		addMouseListener(new MouseAdapter()
			...
			public void mouseClicked(MouseEvent e)
				...
				if ((doControls = !doControls))
					...
					{
					try { surf.sleepAmount = Long.parseLong(tf.getText().trim()); }
					catch (Exception ex) {}
					surf.start();
					add(surf);
					}
	...
	public class Surface
    	...
    	public void run() {
            ...
            while (thread == me && isShowing()) {
                ...
                if (MemoryMonitor.cbLogUsage.isSelected()) {
                     log(usedStr);
                }
            }
            thread = null;
	...
	protected final void log(String fmt, Object... args)
		{
		SBLog.write("Memory Monitor", String.format(fmt,args));
		}
